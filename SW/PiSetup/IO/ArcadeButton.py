...
# Handles the buttons. Each button also has a light
...
class ArchadeButton(object):
    ...
    def update(self):
        ...
        if not self._gpio.input(self._inputpin):  # Button pressed, PullUp is released
            self._pressed = True
    ...
    def __del__(self):
        self.activate(False)
        print("LedIndicator object deactivated and deleted for IO: " + str(self._outputpin), + " and " +
              str(self._inputpin))


if __name__ == '__main__':
    print("Testcode Arcade Button")

    GPIO.setmode(GPIO.BCM)
    arcades = {"GREEN": ArchadeButton(GPIO, 8, 7, 0.5, 2), "YELLOW": ArchadeButton(GPIO, 14, 15, 0.5, 2),
               "RED": ArchadeButton(GPIO, 23, 24, 0.5, 2), "BLUE": ArchadeButton(GPIO, 20, 21, 0.5, 2)}

    for button in arcades:
        arcades[button].initialize()
        arcades[button].activate(True)
        time.sleep(1)

    try:
        while True:
            for button in arcades:
                state = arcades[button].update()
                if state != "Released":
                    print(button + ": " + state)
            time.sleep(0.1)

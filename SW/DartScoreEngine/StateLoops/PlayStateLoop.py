...
class PlayStateLoop(StateLoop.StateLoop):
    ...
    def initialize(self ):
        ...
        self._setscore = 0
        self._totalscore = 0
        self._statestruct = {"player1": {"d1": "-", "d2": "-", "d3": "-", "set": "-", "total": "-", "diff": "-",
                                         "done": False}}
    ...
    def update(self, frame, context):
        ...
        # TODO: a complete refactor of this state-machine. Its very messy...
        if not self._inited:
            ...
            if self._dartDetector.board_empty(frame):
                self._hits = []
                self._detecteddarts = 0
                self._setscore = 0
                # Reset statestruct
                ...
                self._empty = False
                if self._dartDetector.board_changed(frame, self._previousFrame):
                    ...
                    if self._dartDetectorFrames < dartconfig["DartHit"]["DartHitFrames"]:
                        ...
                        if not self._dartevallocked:
                            if self._dartDetector.detect_dart(frame, self._previousboardstate):
                                score = self._dartDetector._lastscore
                                self._detecteddarts = self._detecteddarts + 1
                                # TODO: Update state-structure here...
...
if __name__ == "__main__":
    ...
    cap.set_transform_matrix(transform)
    stopped = False
